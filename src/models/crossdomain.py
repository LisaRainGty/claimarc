...
def main():
    ...
    for k, v in dict(no_cl=False, no_fusion=False, n_fusion=2, fusion_dropout=0.2,
                     ...
                     tag=f"xdom_{args.holdout}").items():
        setattr(args, k, v)
    ...
    if sum(r["y"] for r in held) < 3:
        print("RESULT_XDOM", json.dumps({"holdout": args.holdout, "skip": "too_few_pos"}), flush=True)
        return
    ...
    thr = res["thr"]
    out = {"holdout": args.holdout, "forward_macro_f1": round(macro_f1(hy[query],
